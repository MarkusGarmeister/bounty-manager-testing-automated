...
test.use({
  ...
  test("Curator Proposal", async ({ webPage, context }) => {
    ...
    await mbp.bountyManagerLogo.click();
    await webPage.reload();
    const firstBountyHeader = webPage.locator('.text-white .flex.justify-between .flex.flex-col.lg\\:flex-row.lg\\:justify-start span.text-xl').first();
    await expect(firstBountyHeader).toBeVisible();
    const firstBountyText = await firstBountyHeader.innerText();
    await expect(firstBountyHeader).toContainText(`#${global.bountyId}`);
    const formattedBountyHeader = firstBountyText.replace(/\s+/g, ' ').trim();
    expect(formattedBountyHeader).toContain(`#${global.bountyId}`);

    await expect(mbp.acceptCuratorRole).toBeHidden();
